...
void CClassView::InsertRootItem(const wchar_t* NewName)
{
	hClassViewRoot = m_wndClassView.InsertItem(NewName, 0, 0);
	_ASSERT(hClassViewRoot);
}

void CClassView::InsertItem(void* ActorPointer, const wchar_t* NewName, int nImage, int nSelectedImage, _In_ HTREEITEM hParent)
{	
...
void CClassView::DeleteAllItems()
{
	m_wndClassView.DeleteAllItems();
	Actors.clear();
}

void CClassView::OnSelectedItemChanged(HTREEITEM NewSelectedItem)
{
	...
	void* ActorAddress = It->second;
	theApp.OnActorSelected_Details(ActorAddress);
}

void CClassView::FillClassView()
{
	//hClassViewRoot = m_wndClassView.InsertItem(_T("Map"), 0, 0);
	//m_wndClassView.SetItemState(hClassViewRoot, TVIS_BOLD, TVIS_BOLD);

	//HTREEITEM hClass = m_wndClassView.InsertItem(_T("CFakeAboutDlg"), 1, 1, hClassViewRoot);
	//m_wndClassView.InsertItem(_T("CFakeAboutDlg()"), 3, 3, hClass);

	//m_wndClassView.Expand(hClassViewRoot, TVE_EXPAND);
	/*hClass = m_wndClassView.InsertItem(_T("CFakeApp"), 1, 1, hClassViewRoot);
	m_wndClassView.InsertItem(_T("CFakeApp()"), 3, 3, hClass);
	m_wndClassView.InsertItem(_T("InitInstance()"), 3, 3, hClass);
	m_wndClassView.InsertItem(_T("OnAppAbout()"), 3, 3, hClass);

	hClass = m_wndClassView.InsertItem(_T("CFakeAppDoc"), 1, 1, hClassViewRoot);
	...
	hClass = m_wndClassView.InsertItem(_T("CFakeAppView"), 1, 1, hClassViewRoot);
	...
	hClass = m_wndClassView.InsertItem(_T("CFakeAppFrame"), 1, 1, hClassViewRoot);
	...
	hClass = m_wndClassView.InsertItem(_T("Globals"), 2, 2, hClassViewRoot);
...
void CMyViewTree::OnTvnSelchanged(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	CClassView* WorldOutliner = static_cast<CClassView*>(GetParent());
